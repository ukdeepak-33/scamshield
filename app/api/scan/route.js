...
export async function POST(request) {
  try {
    ...
    // Run fresh scan every time (no cache) — accuracy over speed
    const result = await runScan(url.trim())

    const { data: existing } = await supabaseAdmin.from('sites').select('id, scan_count').eq('domain', domain).single()

    const { data: site, error: siteError } = await supabaseAdmin.from('sites').upsert({
    ...
    if (result.flags.length > 0) {
      ...
      await supabaseAdmin.from('flags').insert(
        result.flags.map(f => ({
          site_id: site.id, category: f.category || 'general',
          label: f.label, detail: f.detail, severity: f.severity, icon: f.icon
        }))
      )
    }

    return NextResponse.json({ ...site, flags: result.flags, fromCache: false })
  } catch (err) {
